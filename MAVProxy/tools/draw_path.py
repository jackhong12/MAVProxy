...
def create_imagefile(options, filename, latlon, ground_width, path_objs, mission_obj, fence_obj, kml_objects, width=600, height=600, used_flightmodes=[], mav_type=None, labels=[]):
    ...
    tuples = [('waypoint', (220, 220, 220))]
    for i in range(len(labels)):
        tuples += [(labels[i], color_list[i % len(color_list)])]
...
def mavflightview(options, args):
    path_list = []
    label_list = []
    for i in range(len(args) // 2):
        filename = args[2 * i]
        label_list += [args[2 * i + 1]]
    ...
    for i in range(len(path_list)):
        if len(path_list[i]) != 0:
            path_objs.append(mp_slipmap.SlipPolygon('FlightPath[%u]-%s' % (i,title), path_list[i], layer='FlightPath',
                                                    linewidth=1, colour=color_list[i % len(color_list)]))
    plist = []
    ...
    if len(plist) > 0:
        ...
        for i in range(len(plist)):
            mission_obj.append(mp_slipmap.SlipPolygon('Mission-%s-%u' % (title,i), plist[i], layer='Mission',
                                                      linewidth=1, colour=(220,220,220)))
    else:
        mission_obj = None
    ...
    if options.imagefile:
        create_imagefile(options, options.imagefile, (lat,lon), ground_width, path_objs, mission_obj, fence_obj, kml_objects, width=width, height=height, used_flightmodes=used_flightmodes, mav_type=mav_type, labels=label_list)
    else:
        global multi_map
...
if __name__ == "__main__":
    ...
    if len(args) < 1:
        print("Usage: mavflightview.py [options] <LOGFILE> <LABEL> ...")
        sys.exit(1)
